-- ============================================
-- KPI: Order Completion Rate
-- ============================================
-- Description: Percentage of orders that reach completion status
-- Temporal Grain: Daily, Weekly, Monthly
-- Business Target: >90% (lower rates indicate checkout/payment issues)

WITH order_status_counts AS (
    SELECT
        DATE(order_date) AS order_day,
        strftime('%Y-W%W', order_date) AS order_week,
        strftime('%Y-%m', order_date) AS order_month,
        order_status,
        COUNT(*) AS status_count
    FROM fact_orders
    GROUP BY 
        DATE(order_date),
        strftime('%Y-W%W', order_date),
        strftime('%Y-%m', order_date),
        order_status
),
daily_metrics AS (
    SELECT
        order_day AS period,
        'daily' AS grain,
        SUM(CASE WHEN order_status = 'completed' THEN status_count ELSE 0 END) AS completed_orders,
        SUM(status_count) AS total_orders
    FROM order_status_counts
    GROUP BY order_day
),
weekly_metrics AS (
    SELECT
        order_week AS period,
        'weekly' AS grain,
        SUM(CASE WHEN order_status = 'completed' THEN status_count ELSE 0 END) AS completed_orders,
        SUM(status_count) AS total_orders
    FROM order_status_counts
    GROUP BY order_week
),
monthly_metrics AS (
    SELECT
        order_month AS period,
        'monthly' AS grain,
        SUM(CASE WHEN order_status = 'completed' THEN status_count ELSE 0 END) AS completed_orders,
        SUM(status_count) AS total_orders
    FROM order_status_counts
    GROUP BY order_month
)
SELECT
    period,
    grain,
    completed_orders,
    total_orders,
    ROUND(completed_orders * 100.0 / total_orders, 2) AS completion_rate_pct
FROM daily_metrics

UNION ALL

SELECT
    period,
    grain,
    completed_orders,
    total_orders,
    ROUND(completed_orders * 100.0 / total_orders, 2) AS completion_rate_pct
FROM weekly_metrics

UNION ALL

SELECT
    period,
    grain,
    completed_orders,
    total_orders,
    ROUND(completed_orders * 100.0 / total_orders, 2) AS completion_rate_pct
FROM monthly_metrics
ORDER BY grain, period;

